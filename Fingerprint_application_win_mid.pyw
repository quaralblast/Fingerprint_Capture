...
class MyGUI():
    def __init__(self):
        ...
        # List Cameras (from Patrick Yeadon via Stack Overflow)
        index = 0
        self.camera_options = []
        while True:
            ...
            if not self.cap.read()[0]:
                ...
                self.camera_options.append(index)
            self.cap.release()
            index += 1
        
        print(self.camera_options)
        # Open Camera (Second argument describes backend)
        self.cap = cv2.VideoCapture(self.camera_options[0], cv2.CAP_DSHOW) # cv2.CAP_V4L2 for linux, cv2.CAP_DSHOW for windows testing
        ...
        self.camera_options_cb = ttk.Combobox(self.top_frame, values=self.camera_options)
    ...
    def set_camera(self):
        camera_index = int(self.camera_options_cb.get())
