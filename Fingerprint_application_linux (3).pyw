...
# Use pkg_resources to check for installed versions
try:
...
def ensure_package(pkg_name, version_spec=None):
    """
    Ensure that pkg_name (optionally with version_spec, e.g. "<2.0.0") is installed.
    If missing or wrong version, pip-install it.
    """
    requirement = f"{pkg_name}{version_spec}" if version_spec else pkg_name
    try:
        pkg_resources.require([requirement])
    except (pkg_resources.DistributionNotFound, pkg_resources.VersionConflict):
        print(f"[BOOTSTRAP] Installing {requirement}...")
        subprocess.check_call([sys.executable, "-m", "pip", "install", requirement])

# === Pin numpy below v2 to avoid 1.x vs 2.x conflicts ===
ensure_package("numpy", "<2.0.0")

# === Other core dependencies ===
for pkg in ("opencv-python", "Pillow", "rembg"):
    ensure_package(pkg)

# === Raspberry Pi GPIO only on ARM/Linux ===
if platform.system() == "Linux" and platform.machine().startswith(("arm", "aarch")):
    ensure_package("RPi.GPIO")
else:
    try:
        import RPi.GPIO as GPIO
    except ImportError:
        GPIO = None
        print("[BOOTSTRAP] RPi.GPIO not available; GPIO features will be disabled.")

# --- Ensure tkinter is available (on Linux it’s a system package) ---
try:
    import tkinter
except ImportError:
    if platform.system() == "Linux":
        print("[BOOTSTRAP] tkinter not found. Installing python3-tk via apt…")
        subprocess.check_call(["sudo", "apt-get", "update"])
        subprocess.check_call(["sudo", "apt-get", "install", "-y", "python3-tk"])
    else:
        print("Error: tkinter not found. Please install it manually for your OS.")
        sys.exit(1)
